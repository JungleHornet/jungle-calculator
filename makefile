build:
	...
	cd cmd/jcalc && tar -c -z -f /build/jungle-calculator-linux64.tar.gz build/jungle-calculator
	cd cmd/jcalc && GOOS=linux GOARCH=arm64 go build -o build/jungle-calculator .
	cd cmd/jcalc && tar -c -z -f /build/jungle-calculator-linux_arm64.tar.gz build/jungle-calculator
	cd cmd/jcalc && GOOS=darwin GOARCH=amd64 go build -o build/jungle-calculator .
	cd cmd/jcalc && tar -c -z -f /build/jungle-calculator-macos64.tar.gz build/jungle-calculator
	cd cmd/jcalc && GOOS=darwin GOARCH=arm64 go build -o build/jungle-calculator .
	cd cmd/jcalc && tar -c -z -f /build/jungle-calculator-macos_arm64.tar.gz build/jungle-calculator
