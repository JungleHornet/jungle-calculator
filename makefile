build:
	...
	cd cmd/jcalc && tar cz -f build/jungle-calculator-linux64.tar.gz build/jcalc
	cd cmd/jcalc && GOOS=linux GOARCH=arm64 go build -o build/jcalc .
	cd cmd/jcalc && tar cz -f build/jungle-calculator-linux_arm64.tar.gz build/jcalc
	cd cmd/jcalc && GOOS=darwin GOARCH=amd64 go build -o build/jcalc .
	cd cmd/jcalc && tar cz -f build/jungle-calculator-macos64.tar.gz build/jcalc
...
testbuild:
	cd cmd/jcalc && GOOS=linux GOARCH=amd64 go build .
	cd cmd/jcalc && GOOS=linux GOARCH=arm64 go build .
	cd cmd/jcalc && GOOS=darwin GOARCH=amd64 go build .
	cd cmd/jcalc && GOOS=darwin GOARCH=arm64 go build .
	cd cmd/jcalc && GOOS=windows GOARCH=amd64 go build .
	cd cmd/jcalc && GOOS=windows GOARCH=arm64 go build .
