build:
	...
	cd cmd/jcalc && tar czf build/jcalc /build/jungle-calculator-linux_arm64.tar.gz
	cd cmd/jcalc && GOOS=darwin GOARCH=amd64 go build -o build/jcalc .
	cd cmd/jcalc && tar czf build/jcalc /build/jungle-calculator-macos64.tar.gz
	cd cmd/jcalc && GOOS=darwin GOARCH=arm64 go build -o build/jcalc .
	cd cmd/jcalc && tar czf build/jcalc /build/jungle-calculator-macos_arm64.tar.gz
